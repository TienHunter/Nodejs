...
let getProfileDoctorByDoctorId = async (req, res) => {
   try {
      let data = await doctorService.getProfileDoctorByDoctorId(req.query.doctorId);
      return res.status(200).json(data)
   } catch (error) {
      console.log('error et profile doctor by id: ', error)
      return res.status(200).json({
         errCode: -1,
         errMessage: 'Error from server'
      })
...
let getListPatientsOfDoctor = async (req, res) => {
   try {
      let data = await doctorService.getListPatientsOfDoctor(req.query.doctorId, req.query.date);
      return res.status(200).json(data)
   } catch (error) {
      console.log(error);
      return res.status(200).json({
